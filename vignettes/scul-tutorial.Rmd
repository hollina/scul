...
In general we want to pick the prediction that captures the underlying data generating process *before* treatment occurs, since we are using this prediction to evaluate a counterfactual as if treatment had not occured. Treatment (if it has any effect) will impact the underlying data generating process. 
...
In short, cross-validation is an organized way of choosing between penalty parameters where the objective is to find weights that best match the underlying factors during the pre-treatment period **and** that will provide an acceptable out-of-sample predcition. 
...
The procedure preforms as many cross-validation runs as possible given the `PrePeriodLength` and the user input desired length of training (`NumberInitialTimePeriods`) and length of testing (`NumberInitialTimePeriods`). 
...
- `y.actual`: The obvserved target series
...
- `coef.exact`: A matrix of the coefficients for each donor variable which are used to create the synthetic prediction. We present these in a slighly unconventional manner outlined later in the vignette. 
...
In the table below, we report measures of pre-period fit and treatment effect estimates for five potential synthethic control estimators. Treatment effect estimates are simply the average difference between the actual and counterfactual SCUL prediction.
...
In the first figure, we compare the pseuedo differences between each placebo and its synthetic control. The graph only includes placebo lines that survived the Cohen's D screen by having a pre-treatment Cohen's D less than 0.25. The placebo lines in the graph are drawn with some transparency so that the darker areas have a greater density of placebo units than lighter spaces. 
...
- `StartTime` and `EndTime`: These form the begining and end time periods over which the average pseduo treatment effect is calculated. This is usful if you want to consider smaller intervals of time than the entire post-treatment period. 
...
However, we have contructed our data in such a way that we can conduct a version of the traditional approach. Here the *summary statistics* will be 
...
- The orange dashed line is the difference using the tradtional prediction
...
We also display the traditional weights as well as the share each donor contributes to the first and last prediction (our perferred way of displaying weights). All donors here are only cigarette sales in other states. The number in the first column is the FIPS code for each donor.

Unlike the weights for SCUL (displayed earlier), the weights for the traditional method are all positive and sum to one. However these weights are not the same as the share the variable contributes to the prediction. Take the first donor (Kentuck) for example. The coefficient on Kentucky cigarette sales is 0.3229, but in the most recent prediction the share this donor is responsible for is 52.11%. 

Also of note. Many of these variable recieve nearly, but not exactly zero weight. The SCUL procedure produces a more sparse set of donors since the majority of the donors receive *exactly* zero weight. 
...
- When there are more donors than time periods (i.e., high-dimensiona data)
- When the target series is outside the the support of the donor pool (case 1 below)
- When negatively correllated donors can help identify underlying data generating process (case 2 below)

The SCUL procedure is a flexible synthetic control method that accomodates all of these scenarios. 
