...
#' 
#' @export
PlotShareForFirstTopTen <- function(
  ...
  # Plot the top ten results for the first time
  ShareForFirstTopTen <- ggplot(data=ShareOfPredictionForPlot[1:10,],aes(share.1, order,label=RowNames)) +
    geom_point() + 
    geom_text(data = ShareOfPredictionForPlot[1:10,], aes( label = ShareOfPredictionForPlot$RowNames[1:10]),
              size = 6, hjust = -.15) +
    ...
    ylab("") + xlab("Percent [0-100]") +
    ggtitle("Contribution of Top Ten Donor Variables to Synthetic Prediction in the First time of Treatment") +
    scale_x_continuous(limits = c(0, max(ShareOfPredictionForPlot$share.1[1:10])+.1) ) +
  ...
  # Save graph
  ShareForFirstTopTenPath<-paste(OutputFilePath,"share_for_first_time_top_ten.pdf",sep='')
  ggsave(ShareForFirstTopTenPath, width=12, height=8, dpi=300)
  ####################
  ## Return plot
  return(ShareForFirstTopTen)
}