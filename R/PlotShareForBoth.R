...
PlotShareForBoth <- function(
  ...
  #x.DonorPoolWithIntercept <- as.matrix(rep(1,nrow(x.DonorPool)),x.DonorPool)
  ...
  ShareOfPredictionForPlot$order<-""
  ...
  ShareOfPredictionForPlot.long$RowNames[ShareOfPredictionForPlot.long$time_period=="share.1"] <- ""
  ...
  # Make sure that order is a number 
  ShareOfPredictionForPlot.long$order <- as.numeric(ShareOfPredictionForPlot.long$order)
  
  # Initialize the graph
  graph <- ggplot(data=ShareOfPredictionForPlot.long,aes(x=share, y=order,label=RowNames, color=factor(sign)))
  ...
  for (i in seq(from=5, to=nrow(ShareOfPredictionForPlot.long), by=5)) {
    graph <- graph +
      geom_segment(x=-0.01, y=i, xend=max(ShareOfPredictionForPlot.long$share), yend=i, linetype="dashed", color="grey")
  }
  ...
  graph <- graph + 
    ...
    geom_text(data = ShareOfPredictionForPlot.long, aes(x=ShareOfPredictionForPlot.long$zero,y=ShareOfPredictionForPlot.long$order, label = ShareOfPredictionForPlot.long$RowNames), size = 6, adj=1, color="black")
  ...
  graph <- graph +
    ...
    theme(axis.title.y=element_blank(),axis.text.y=element_blank(),axis.line.y=element_blank(),axis.ticks.y=element_blank()) +scale_x_continuous(limits = c(-.075, max(ShareOfPredictionForPlot.long$share))) 
  ...
  ShareForBoth <- graph  +
    ...
    guides(fill = guide_legend(override.aes=list(shape=22),title.position = "top",title.theme = element_text(size = 20,angle = 0))) +
    guides(shape = guide_legend(title.position = "top",title.theme = element_text(size = 20,angle = 0))) +
  ...
  # Save graph
  ShareForBothPath<-paste(OutputFilePath,"share_for_both_time.pdf",sep='')
  ggsave(ShareForBothPath, width=12, height=8, dpi=300)
  ####################
  ## Return plot
  return(ShareForBoth)
}