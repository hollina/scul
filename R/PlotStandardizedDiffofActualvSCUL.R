...
#' @import ggplot2
#' @export
PlotStandardizedDiffOfActualvSCUL <- function(
  ...
  TempStandardizedDiffPlot <- ggplot() + 
    geom_line(data=DataToPlot, aes(x=time,y = StandardizedDifference,color="Standardized difference between actual and synthetic"), size=.5, linetype="solid") +
    scale_color_manual(name = "", values = c("Standardized difference between actual and synthetic" = "black", "b" = "red"))+  theme_bw(base_size = 22)  +
    theme(panel.border = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"))  
  
  StandardizedDiffPlot <- TempStandardizedDiffPlot   +
    geom_vline(xintercept =DataToPlot$time[TreatmentBeginsAt-1], colour="grey", linetype = "dashed")  +
    geom_hline(yintercept=0, color = "red") +
    theme(legend.position="bottom") + 
    ylab("")  +
    xlab("Time") +
    ggtitle("Standardized difference between actual and synthetic prediction") +
    annotate("text", x = (DataToPlot$time[TreatmentBeginsAt-1]-DataToPlot$time[1])/2+DataToPlot$time[1], y = max(DataToPlot$StandardizedDifference)*1.05, label = "Pre-treatment",cex=6) +
    annotate("text", x = (DataToPlot$time[length(y.actual)]-DataToPlot$time[TreatmentBeginsAt-1])/2+DataToPlot$time[TreatmentBeginsAt-1], y = max(DataToPlot$StandardizedDifference)*1.05, label = "Post-treatment",cex=6)
  
  # Show the graph
  StandardizedDiffPlot
  
  # Save graph
  StandardizedDiffPlotPath<-paste(OutputFilePath,"standardized_diff_actual_v_synthetic_plot.pdf",sep='')
  ggsave(StandardizedDiffPlotPath, width=12, height=8, dpi=300)
  ####################
  ## Return plot
  return(StandardizedDiffPlot)
}